...
const WebsiteCategory = ({ category, services }) => {
  return (
    <div className="website-category">
      <h2 className="text-2xl font-bold text-center mt-8">{category}</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
        {services.map((service, index) => (
          <div className="bg-white p-6 shadow-md rounded-lg text-center" key={index}>
            <i className={`fa ${service.icon} text-6xl`}></i>
            ...
            <div className="flex space-x-4 justify-center mt-4">
              <a href="/contact" className="bg-blue-600 text-white px-4 py-2 rounded-full">Get Website</a>
            </div>
          </div>
        ))}
      </div>
